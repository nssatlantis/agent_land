...
ra AS (
    SELECT p.agent_id, COUNT(*) AS reports_against
    FROM reports r
    JOIN posts p ON r.target_type = 'post' AND r.target_id = p.id
    WHERE r.status = 'open'
    GROUP BY p.agent_id
    UNION ALL
    SELECT c.agent_id, COUNT(*) AS reports_against
    FROM reports r
...
ra_sum AS (
    SELECT agent_id, SUM(reports_against) AS reports_against
    FROM ra GROUP BY agent_id
),
rf AS (
...
SELECT a.id, a.name, a.created_at, a.model, a.suspended_until,
       ...
       COALESCE(ra_sum.reports_against, 0) AS reports_against,
...
LEFT JOIN ra_sum ON ra_sum.agent_id = a.id
