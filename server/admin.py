...
def _render_posts_manager(request) -> str:
    ...
    if q_lower:
        filtered = [
            p for p in filtered
            if q_lower in (p["title"] or "").lower() or q_lower in (p["author"] or "").lower()
        ]

    # Tabs
    tabs = ""
    for key, label in [("all", "All"), ("post", "Ordinary"), ("proposal", "Proposals"), ("small_fix", "Small fixes"), ("idea", "Ideas")]:
    ...
    for p in filtered[:100]:
        ...
        collab_badge = ' <span style="background:#7c3aed;color:white;padding:1px 6px;border-radius:999px;font-size:11px">collab</span>' if p.get("collaborative") else ""
        closed_badge = ""
        ...
        claim_badge = ' <span style="background:#0ea5e9;color:white;padding:1px 6px;border-radius:999px;font-size:11px">claimable</span>' if p.get("claimable") else ""
        delegate_note = f' delegate:{esc(p["delegate_name"])}' if p.get("delegate_name") else ""
        max_coll_note = ""
        ...
        locked_note = f' <span style="color:#c53030">locked by #{p["superseded_by_id"]}</span>' if p.get("superseded_by_id") else ""
...
async def posts_index(request):
    ...
    return _admin_page(request, "admin - posts",
                       _admin_nav() + _render_posts_manager(request))
...
async def admin_update_post_settings(request):
    ...
    if cur.get("proposal_kind") is None:
        return _flash(request, f"post #{post_id} is not a proposal - no proposal settings to change.")
    if cur.get("superseded_by_id") is not None:
        return _flash(request, f"proposal #{post_id} is locked by #{cur['superseded_by_id']} - settings are frozen.")
    applied = []
    ...
    if wants_close:
        ...
        return RedirectResponse(request.headers.get("referer") or "/admin/posts", status_code=303)
    if wants_reopen:
        try:
    ...
    if "collaborative" in form:
        try:
            ...
            if bool(cur["collaborative"]) != wanted_collab:
                ...
                with db._conn() as conn:
                    cur = dict(conn.execute("SELECT collaborative, collaborative_closed FROM posts WHERE id = ?", (post_id,)).fetchone())
    ...
    if "claimable" in form:
        try:
            ...
            with db._conn() as conn:
                cur_claim = conn.execute("SELECT claimable FROM posts WHERE id = ?", (post_id,)).fetchone()
    ...
    if "max_collaborators" in form:
        ...
        try:
            if raw == "":
                ...
                with db._conn() as conn:
                    prow = conn.execute("SELECT proposal_config FROM posts WHERE id = ?", (post_id,)).fetchone()
                    has_cap = False
    ...
    if "pr_goal" in form:
        ...
        try:
            if raw == "":
                with db._conn() as conn:
                    cur_g = conn.execute("SELECT pr_goal FROM posts WHERE id = ?", (post_id,)).fetchone()
    ...
    if "delegate" in form:
        ...
        try:
            ...
            with db._conn() as conn:
                cur_d = conn.execute("SELECT delegate_id FROM posts WHERE id = ?", (post_id,)).fetchone()
