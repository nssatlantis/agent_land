            ...
            try:
                if os.path.isdir(d):
                    ...
                    for p in Path(d).rglob("*"):
                        ...
                        except Exception:  # domain: degrade-silently - dashboard best-effort, stat
                            pass
                        if total > 500 * 1024 * 1024:
                            break
                    size = f"{total // (1024*1024)}M"
            except Exception:  # domain: degrade-silently - dashboard best-effort, size calc
                pass
            ci_details.append(
                {
                    "idx": i,
    ...
    try:
        ...
        def _docker_images():
            ...
            for line in ls.stdout.splitlines():
                ...
                if parts[0]:
                    out.append({"tag": parts[0], "size": parts[1] if len(parts) > 1 else ""})
            return out[:10]
...
def _render_ci_dashboard(request) -> str:
    ...
    ci_html = (
        ...
        + "</table></div>"
        + ("<p style=color:var(--muted)>" + esc(ci["error"]) + "</p>" if "error" in ci else "")
        + "</div>"
    )
    ws_html = (
        ...
        + "</table></div>"
        + ("<p style=color:var(--muted)>" + esc(ws["error"]) + "</p>" if "error" in ws else "")
        + "</div>"
    )
    # Ticker
    ...
    ticker_html = (
        ...
        + pending_rows
        + "</table></div>"
        + ("<p style=color:var(--muted)>"+esc(ticker["error"])+"</p>" if "error" in ticker else "")
        + "</div>"
    )
    # Recent
    recent_rows = ""
    ...
    if not recent_rows:
        recent_rows = '<tr><td colspan=5 style="color:var(--muted)">no recent CI events</td></tr>'
    recent_html = (
...
async def ci_admin_page(request):
    if not _authorized(request):
        return _denied()
    return _admin_page(request, "admin - ci", _admin_nav() + _render_ci_dashboard(request))
...
async def ci_prune_images(request):
    ...
    try:
        ...
        ls = subprocess.run(
            ["docker", "image", "ls", "--format", "{{.Repository}}:{{.Tag}}", "--filter", f"reference={_cfg.CI_RUN_IMAGE_BASE}:*"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        kept = 0
        for t in tags[1:]:
            pr = subprocess.run(["docker", "rmi", "-f", t], capture_output=True, text=True, timeout=30)
            if pr.returncode == 0:
                kept += 1
