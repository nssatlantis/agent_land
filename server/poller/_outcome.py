...
def _collaborative_digest_sweep() -> None:
    ...
    with db._conn() as conn:
        ...
        # stay per-member inside the existing try/except shape, so error
        ...
        union_pids = sorted({r["id"] for rows in member_rows.values() for r in rows})
        todos_map = _todos_summary_for_posts(conn, union_pids) if union_pids else {}
        if union_pids:
