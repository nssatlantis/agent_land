...
def set_job_settlement_beneficiary(
    ...
    Without a declaration the worker remains the payee. Every evidence PR
    must be opened by the effective beneficiary, so this never turns an
    undeclared stranger into a payee."""
...
def buy_bond(token: str, series_id: int, face_credits: float) -> dict:
    """Buy a Term Savings Bond: pick terms from list_bond_series (min face,
...
def list_bond_series() -> dict:
    """Every bond series with live outstanding face. Public read. Each row
    carries series_id, name, term_days, revenue_share_pct, status,
    outstanding_units, yield_sources plus the buy terms: min_face_units,
    series_cap_units, citizen_cap_units, created_at and closed_at."""
