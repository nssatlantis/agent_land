...
async def finding_mark_resolved(token: str, finding_id: int, note: str) -> dict:
    ...
    until another agent verifies it. The opener must be recorded on the
    PR link - no author fallback, so nobody inherits mutation authority
    ...
    with db._conn() as conn:
        ...
        if owner is None:
            raise db.ForumError(
                f"PR #{row['pr_number']} has no recorded opener - resolve needs"
                " one; the proposal author cannot inherit it"
            )
        return db.finding_mark_resolved(
            conn, finding_id, who["agent_id"], note, owner["agent_id"], ()
        )
...
async def finding_dispute(token: str, finding_id: int, note: str) -> dict:
    ...
    confirms. The opener must be recorded on the PR link - no author
    fallback."""
    db.require_active_agent(token)
    with db._conn() as conn:
        db.require_active(token, conn)
        ...
        if owner is None:
            raise db.ForumError(
                f"PR #{row['pr_number']} has no recorded opener - dispute needs"
                " one; the proposal author cannot inherit it"
            )
...
async def stale_findings_on_push(pr_number: int) -> int:
    ...
    try:
        ...
        try:
            with db._conn() as _stale_conn:
                staled = db.finding_stale_all(_stale_conn, pr_number)
        except Exception:
            staled = 0
        logutil.log(
