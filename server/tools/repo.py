...
async def repo_propose_change(
    ...
    if dry_run:
        ...
        try:
            plan["rehearse_hint"] = f"Run repo_ci_run(token, files=[...]) with same {len(changes)} file(s) payload before opening (content_manifest shows bytes/sha256); shares the 2-slot runner pool (ci_local_run) and reports ok/timed_out/exit_code. Example: repo_ci_run(token, files=<same files>)"
        ...
        try:
            ...
            plan["ci_ran"] = ci_ran
            if not ci_ran:
                plan["ci_hint"] = f"No recent CI run in last {window//3600}h — run repo_ci_run(token, files=[...]) with same payload (or tests) before opening to verify. Dry-run rehearsal shares pool and does not block branch CI."
        except Exception:  # domain: degrade-silently
            pass
    else:
        try:
            ...
            for ev in recent:
                if ev["kind"] in ci_kinds:
                    ...
                    if detail.get("ok") is True or detail.get("exit_code") == 0 or not detail:
                        ci_ran = True
                        break
                    ci_ran = True
                    break
            plan["ci_ran"] = ci_ran
            if not ci_ran:
                plan["ci_hint"] = f"No recent CI run in last {window//3600}h — run repo_ci_run(token, files=[...]) with same files payload (or tests) before opening to verify. Shares the 2-slot runner pool (ci_local_run) and reports ok/timed_out/exit_code."
        except Exception:  # domain: degrade-silently - advisory never blocks the PR response
            pass
    return plan
