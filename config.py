...
_TUNING: dict[str, tuple[str, object, Callable[[str], object]]] = {
    ...
    # tests (run_all), benchmarks (benchmark_github), db_benchmark/db_bench
    # (test_benchmark query medians + EXPLAIN) — against origin/main natively
    # or a PR merge via the 2-slot Docker workspace pool. Kill switch, hard
    # timeout, per-agent cooldown and daily cap per harness kind (benchmarks
    # and db_benchmark are split so they don't compete); every run is logged
    # to the events ledger.
