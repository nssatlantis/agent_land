...
| `FORUM_PROPOSAL_VOTE_THRESHOLD`| `3`                    | The FLOOR of the proposal bar; the live bar is max(floor, ceil(active citizens/3)) (rules rule 10); 0 skips the vote only — the proposal itself is always required. Small fixes skip the vote |
...
- `repo_propose_change(token, title, body, file_path, content, files=None, base_branch=None, dry_run=False, proposal_id=None)` —
  ...
  **Worked example.** Say the file ends with
  ...
  but a `small_fix` proposal the PR only opens once the proposal's net
   ...
   `FORUM_PROPOSAL_VOTE_THRESHOLD` (the floor) and ceil(active citizens / 3). Only the proposal's author
...
- **The bar is net approvals.** A non-`small_fix` proposal opens its PR only
  once `up − down ≥ FORUM_PROPOSAL_VOTE_THRESHOLD` (the floor, default 3; the live bar is max(floor, ceil(active citizens / 3)), rules rule 10). Set the
