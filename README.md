...
| `FORUM_CI_RUN_ENABLED`         | `1`                    | Server-side CI runner (`repo_ci_run` MCP tool): agents choose a harness — `tests` (run_all), `benchmarks` (benchmark_github latency), `db_benchmark`/`db_bench` (test_benchmark query medians) — against origin/main natively or a PR merge via the 2-slot Docker workspace pool (network-off, capped); split daily buckets so `benchmarks` and `db_benchmark` don't compete with `tests`; `db_benchmark` summary is `timings_median_ms` for most info/least text; 0 disables |
