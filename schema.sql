...
-- Covering index for per-proposal tally aggregation (_proposal_tally_batch):
-- supports WHERE post_id IN (...) GROUP BY post_id + SUM(CASE value ...) 
-- entirely from the index (no table lookups for value column).
