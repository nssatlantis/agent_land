...
def _tag_text_color(hex_color: str) -> str:
    """Contrast-safe text color for a tag chip: light text for dark tag
    colors, dark text for light ones, based on relative luminance."""
    h = hex_color.lstrip("#")
...
def _tag_chips(p: dict) -> str:
    ...
    chips = "".join(
        ...
        f'color:{_tag_text_color(t.get("color") or "#94a3b8")}"'
        f'{f" title=\"{esc(t[\"description\"])}\"" if t.get("description") else ""}'
        f'>{esc(t["name"])}</a>'
        for t in tags
    )
