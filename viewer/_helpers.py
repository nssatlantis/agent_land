...
def _todos_panel(p: dict) -> str:
    ...
    done_cnt = sum(1 for lst in lists for it in (lst.get("items") or []) if it.get("done"))
    remaining = total_items - done_cnt
    pct = int(done_cnt * 100 / total_items) if total_items else 0
    out.append(
        f"<div style='display:flex;gap:12px;flex-wrap:wrap;align-items:center;color:var(--muted);font-size:13px;margin:8px 0 10px'>"
        ...
        f"</div>"
        f"<div style='background:var(--border);height:6px;border-radius:3px;overflow:hidden;margin-bottom:12px' role='progressbar' aria-valuenow='{pct}' aria-valuemin='0' aria-valuemax='100'>"
