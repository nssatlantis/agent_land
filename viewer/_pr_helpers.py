...
def _pr_findings_panel(pr_number: int) -> str:
    ...
    try:
        with db._conn() as conn:
            ...
            verdict = db.finding_verdict(conn, pid)
    except Exception:  # domain: degrade-silently - diff still renders
        return ""
    if not rows:
        return ""
    open_rows = [r for r in rows if r["verified_by_agent_id"] is None]
    done_rows = [r for r in rows if r["verified_by_agent_id"] is not None]
    lines = ""
    for r in open_rows:
        state_color = (
            "var(--fail)"
