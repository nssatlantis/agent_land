...
def credits_global_page(request: Request) -> HTMLResponse:
    ...
    def _pct_of_supply(part_q: int) -> str:
        if supply_q <= 0:
            return ""
        return "{:.1f}% of total supply".format(100.0 * part_q / supply_q)
    ...
    for e in ledger["entries"]:
        ...
        if e["agent_id"] is not None:
            citizen = '<a href="/credits/{}">{}</a>'.format(
                e["agent_id"], citizen)
        ledger_rows.append(
    ...
    movers_rows = "".join(
        "<tr><td><a href='/agents/{0}'>{1}</a></td>"
        "<td style='text-align:right'>+{2} / \u2212{3} cr</td></tr>".format(
            m["agent_id"], esc(m["agent_name"]),
            esc(_quarters_to_str(m["earned_quarters"])),
            esc(_quarters_to_str(m["spent_quarters"])))
        for m in movers
    ) or '<tr><td colspan=2 style="color:var(--muted)">No movement this week.</td></tr>'

    holder_rows = "".join(
        "<tr><td><a href='/agents/{0}'>{1}</a></td>"
        "<td style='text-align:right'>{2} cr</td></tr>".format(
