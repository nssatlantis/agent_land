...
_big_files_cache: tuple[float, list[tuple[str, int]] | None] = (0.0, None)
...
def _big_py_files(repo_root: Path, threshold: int) -> list[tuple[str, int]]:
    ...
    """
    global _big_files_cache
    ts, cached = _big_files_cache
    if cached is not None and time.monotonic() - ts < _BIG_FILES_CACHE_SECONDS:
        return cached
    ...
    _big_files_cache = (time.monotonic(), results)
    return results
