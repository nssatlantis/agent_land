...
def finding_unfund(
    ...
    Only while the finding is still open with no fix recorded: once a
    fix lands the funds are committed to the quorum outcome (payout on
    quorum, frozen on dispute).  Partial amounts allowed down to the
    funder's own funded balance.  Frozen on locked proposals."""
    from db._credits import release_escrow

    row = _frozen_post_for_finding(conn, finding_id)
    if row["state"] != "open" or row["fixed_by_agent_id"] is not None:
