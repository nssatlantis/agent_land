...
def reopen_bug_report(report_id: int, *, admin: str = "") -> dict:
    """Admin action: reopen a quorum/reporter-closed OR FIXED bug (status
    back to open, resolution cleared). Votes, verifications and duplicates
    stay as history; confidence is untouched. The reporter is told."""
    with _conn(immediate=True) as conn:
        row = conn.execute(
            "SELECT id, status, agent_id FROM bug_reports WHERE id = ?",
            (report_id,),
        ...
        if row["status"] not in ("closed", "fixed"):
            raise ForumError(f"Bug report #{report_id} is {row['status']}, not closed or fixed.")
        conn.execute(
            "UPDATE bug_reports SET status = 'open', decided_at = NULL,"
            " resolution = NULL, resolution_note = NULL, claimed_by = NULL,"
            " claimed_at = NULL, claimed_proposal_id = NULL,"
            " fix_pr = NULL, bounty_job_id = NULL WHERE id = ?",
            (report_id,),
        )
        log_event(
            EVT_BUG_REOPENED,
