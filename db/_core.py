...
def init_db() -> None:
    ...
    with sqlite3.connect(_path) as conn:
        ...
        # proposal that already has a run is never double-started, so this is
        # a no-op on fresh DBs and on every later boot. Only still-openable
        ...
        # is decided, mirroring what close_workflow_for_pr does for
        # poller-processed outcomes, so this backfill and the reconciliation
        # cannot fight each other across boots.
