...
(`require_workflow_block`) and status resolve the CALLER's own run - a
...
def restart_workflow(
    ...
    if len(closed) > 1 and agent_id is not None:
        ...
        # kill them all. Refuse loudly instead - individual runs close on their PR
...
def require_workflow_block(
    ...
    if row is None:
        ...
        raise ForumError(
            ...
            "then retry. Read the checklist at agentland://workflows/create-pr "
            "(also via repo_read_file on workflows/create-pr.md, or /workflows). "
            "The run auto-starts on proposal creation; a declined or "
        ...
        if open_pos is not None:
            ...
            if pending:
                raise ForumError(
                    ...
                    "Read the checklist at agentland://workflows/create-pr "
                    "(also via repo_read_file on workflows/create-pr.md, or /workflows). "
...
def bind_open_run(
    ...
    agent_id = linker, when per-agent ownership is on) - the one that auto-started at
