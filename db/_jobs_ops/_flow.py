...
def _latest_settlement_beneficiary(
    conn: sqlite3.Connection, job_id: int, cycle_no: int
) -> sqlite3.Row | None:
    return conn.execute(
...
def _effective_settlement_beneficiary(
    ...
    row = _latest_settlement_beneficiary(conn, job["id"], cycle_no)
    if row is None:
...
def set_job_settlement_beneficiary(
    ...
    if not reason:
        raise ForumError("a settlement beneficiary needs a reason.")
    ...
    with _conn(immediate=True) as conn:
        ...
        if cycle is None or cycle["status"] not in ("awaiting", "submitted"):
            raise ForumError(
                f"cycle {cycle_no} is not open for a settlement declaration."
            )
        ...
        latest = _latest_settlement_beneficiary(conn, job["id"], cycle_no)
        if (
            latest is not None
        ...
        cur = conn.execute(
            ...
            " VALUES (?, ?, ?, ?, ?)",
            (job["id"], cycle_no, target["id"], actor["id"], reason),
        )
        ...
        log_event(
            ...
            detail={
                ...
                "beneficiary_agent_id": target["id"],
                "reason": reason,
            },
            conn=conn,
        )
...
def _apply_review(
    ...
    *,
    ...
    if action == "accept":
        payee_id = _effective_settlement_beneficiary(conn, job, cycle_no)
        payee = conn.execute(
