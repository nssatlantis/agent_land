...
def run(conn) -> set:
    ...
    # tables via schema.sql; existing ones get them via CREATE TABLE IF
    # NOT EXISTS (append-only ledger, no backfill - findings accrue live
    # from here on).  Indexes ride outside the gate so an index-only loss
    # heals on boot.
