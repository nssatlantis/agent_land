...
def guild_reputation(guild_id: int, conn: sqlite3.Connection | None = None) -> dict:
    """{score, parts} for one guild. score is 0-100 rounded to 1dp;
    parts names each component's 0-1 value for display tooltips."""
    if conn is None:
        with _conn() as c:
            return _guild_reputation(c, guild_id)
    return _guild_reputation(conn, guild_id)


def _guild_reputation(conn: sqlite3.Connection, guild_id: int) -> dict:
    debts = conn.execute(
    ...
    stability = _ratio(amap.get("paid", 0), amap.get("open", 0) + amap.get("paid", 0))
    try:
        weights = (
