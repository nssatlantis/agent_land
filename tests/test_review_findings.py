...
def main():
    ...
    with db._conn() as conn:
        ...
        # PR-scoped findings need a linked PR: anchor 4242 to this proposal.
        conn.execute(
            "INSERT INTO proposal_links (pr_number, post_id, opened_by_agent_id)"
            " VALUES (4242, ?, ?)",
            (pid, alpha),
        )
        # Unlinked and mismatched PR anchors are refused.
        err = expect_error(
            db.finding_add,
            conn,
            pid,
    ...
    with db._conn() as conn:
        ...
        db.finding_mark_resolved(conn, fid4, alpha, "fixed", alpha)
        ...
        err = expect_error(db.finding_dispute, conn, fid4, alpha, "reopen", alpha)
    ...
    async def _fake_pr(number):
        assert number == 4242
        return {"head": {"sha": _SHA_C}}

    github.aget_pr = _fake_pr
    try:
        staled = asyncio.run(ftools.stale_findings_on_push(4242))
    finally:
        github.aget_pr = real_aget
    assert staled == 1, "the hook stales verified rows on push"
    with db._conn() as conn:
        assert len(db.reviewer_blockers(conn, pid, beta)) >= 1, (
    ...
    github.aget_pr = _boom
    try:
        assert asyncio.run(ftools.stale_findings_on_push(4242)) == 0, (
            "a dead GitHub read degrades instead of failing the push"
        )
    finally:
        github.aget_pr = real_aget
