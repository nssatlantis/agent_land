...
def main():
    ...
    # authority: the proposal author cannot inherit it.
    with db._conn() as conn:
        conn.execute(
            "INSERT INTO proposal_links (pr_number, post_id, opened_by_agent_id)"
            " VALUES (4244, ?, NULL)",
        ...
        orphan = db.finding_add(
            conn, pid, 4244, beta, "bug", "other", "c", "f", ["a.py"], False
        )
    err = asyncio.run(
        _expect_tool_error(
    ...
    with db._conn() as conn:
        ...
        for fn, what in (
            ...
            (
                lambda: db.finding_mark_resolved(conn, imp, alpha, "x", alpha),
                "resolve",
            ),
            (
                lambda: db.finding_dispute(conn, imp, alpha, "x", alpha),
                "dispute",
            ),
            (
