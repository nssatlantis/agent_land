...
def main():
    ...
    pid = doomed["post_id"]
    other_comment = db.create_comment(
        helper["token"], pid, "helper comments on the doomed post"
    )
    own_comment = db.create_comment(victim["token"], pid, "victim's own comment")
    db.create_comment(
        helper["token"], pid, "reply", parent_comment_id=own_comment["comment_id"]
    )
    helper_post = db.create_post(helper["token"], "helper post", "h")
    leftover = db.create_comment(
        victim["token"], helper_post["post_id"], "victim on helper's post"
    )
    leftover_reply = db.create_comment(
        helper["token"],
        helper_post["post_id"],
        "reply to victim",
    ...
    db.vote(
        victim["token"], "comment", other_comment["comment_id"], 1
    )  # earns the helper reporting karma
    ...
    assert (
        listing[victim["agent_id"]]["banned"] == 0
        and listing[victim["agent_id"]]["last_ip"] is None
    )
    assert "banned" not in aggregates.list_agents()[0], (
        "the public citizens list must not expose ban state"
    )
    ...
    assert (
        db.whoami(victim["token"])["account_status"] == "banned"
        and db.my_profile(victim["token"])["account_status"] == "banned"
    ...
    assert db.create_post(victim["token"], "x", "y")["post_id"] > 0, (
        "unban restores writes"
    )

    # Manual report resolution: a clear closes the report and the docket shows it.
    moderation.resolve_report(rid, "root", "clear")
    assert (
        next(r for r in reports.list_reports() if r["id"] == rid)["status"] == "cleared"
    )
    ...
    assert "destroy_content" in expect_error(
        moderation.delete_agent, victim["agent_id"], "root"
    )
    ...
    assert (
        moderation.admin_agent_detail
        and next(
            (
                a
                for a in moderation.admin_list_agents()
                if a["id"] == victim["agent_id"]
            ),
            None,
        )
        is None
    ), "deleted agent must vanish from the directory"
    ...
    prop = db.create_proposal(
        proposer["token"], "Proposal: delete me", "body of the proposal"
    )
    pid = prop["post_id"]
    on_prop = db.create_comment(supporter["token"], pid, "supporting comment")
    db.create_comment(
        proposer["token"], pid, "author reply", parent_comment_id=on_prop["comment_id"]
    )
    db.vote(
        proposer["token"], "comment", on_prop["comment_id"], 1
    )  # earns supporter karma
    db.vote(supporter["token"], "post", pid, 1)
    db.vote_on_proposal(supporter["token"], pid, 1)
    prop_report = reports.report_content(
        supporter["token"], "post", pid, "proposal flagged"
    )
    ...
    try:
        ...
        report1 = reports.report_content(
            flagger["token"], "post", victim_post["post_id"], "first flag"
        )
        dup = expect_error(
        ...
        other = reports.report_content(
            voter_a["token"], "post", victim_post["post_id"], "separate flag"
        )
        ...
        assert decided, (
            "a community suspension stamps decided_at on the reports it decides"
        )
        ...
        reports.report_content(
            flagger["token"], "post", fresh_post["post_id"], "different target"
        )
        ...
        with db._conn() as conn:
            conn.execute(
                "UPDATE reports SET decided_at = ? WHERE id = ?",
                (aged, report1["report_id"]),
            )
    ...
    rev = {
        n: db.register_agent(n)
        for n in ("rev-flag", "rev-victim", "rev-voter", "rev-voter2")
    }
    rev_f, rev_v, rev_v1, rev_v2 = (
        rev[n] for n in ("rev-flag", "rev-victim", "rev-voter", "rev-voter2")
    )
    rev_post = db.create_post(rev_v["token"], "rev target", "rev body")
    db.create_comment(rev_v["token"], rev_post["post_id"], "rev comment body")
    rev_comment = db.create_comment(
        rev_v["token"], rev_post["post_id"], "second comment"
    )
    ...
    rp = reports.report_content(
        rev_f["token"], "post", rev_post["post_id"], "rev snap reason"
    )
    ...
    assert rp_detail["target_snapshot"] == {
        "title": "rev target",
        "body": f"rev body\n\n— rev-victim (agent_id={rev_v['agent_id']})",
    }, "a post report freezes its title+body (auto-signature included) at report time"
    assert (
        rp_detail["target_snapshot"]["body"]
        == f"rev body\n\n— rev-victim (agent_id={rev_v['agent_id']})"
    )
    assert rp_detail["target_author"]["karma"] >= 0, (
        "the target author panel carries karma"
    )
    ...
    assert rp_row["target_author"] == "rev-victim", (
        "list_reports carries the flagged author"
    )
    ...
    assert len(reports.list_reports(status="all")) >= len(
        reports.list_reports(status="open")
    )
    ...
    rc = reports.report_content(
        rev_f["token"], "comment", rev_comment["comment_id"], "comment snap"
    )
    ...
    assert (
        rc_detail["target_type"] == "comment"
        and rc_detail["target_id"] == rev_comment["comment_id"]
    )
    ...
    assert {v["action"] for v in resolved["votes"]} == {"suspend", "clear"} or len(
        resolved["votes"]
    ) >= 2, "resolved votes carry identities"
    ...
    with db._conn() as conn:
        ...
        archived = conn.execute(
            "SELECT COUNT(*) FROM report_votes_archive WHERE report_id = ?",
            (rp["report_id"],),
        ).fetchone()[0]
    ...
    rev_admin_comment = db.create_comment(
        rev_v2["token"], rev2_post["post_id"], "admin target comment"
    )
    rclr = reports.report_content(
        rev_f["token"], "comment", rev_admin_comment["comment_id"], "admin clear"
    )
    ...
    sib_a = reports.report_content(
        rev_f["token"], "post", sib_post["post_id"], "sibling A"
    )
    sib_b = reports.report_content(
        rev_v1["token"], "post", sib_post["post_id"], "sibling B"
    )
    assert sib_a["report_id"] != sib_b["report_id"], (
        "two reporters can hold two open reports"
    )
    ...
    del_rep = reports.report_content(
        rev_f["token"], "post", del_post["post_id"], "delete sweep"
    )
    moderation.delete_post(del_post["post_id"], "root")
    survived = reports.get_report(del_rep["report_id"])
    assert survived["status"] == "removed", (
        "a report on deleted content survives as 'removed'"
    )
    assert survived["target_snapshot"] == {
        "title": "rev delete target",
        "body": f"rev delete body\n\n— rev-voter2 (agent_id={rev_v2['agent_id']})",
    ...
    assert any(
        r["status"] == "removed" for r in reports.list_reports(status="resolved")
    ...
    rp2 = reports.report_content(
        rev_f["token"], "post", rev3_post["post_id"], "fresh after removed"
    )
    ...
    com_post = db.create_post(
        rev_v2["token"], "rev community sibling target", "com body"
    )
    com_a = reports.report_content(rev_f["token"], "post", com_post["post_id"], "com A")
    com_b = reports.report_content(
        rev_v1["token"], "post", com_post["post_id"], "com B"
    )
    assert com_a["report_id"] != com_b["report_id"], (
        "two reporters hold two open reports"
    )
    ...
    try:
        ...
        rs_clear_post = db.create_post(rs_a["token"], "rs clear post", "body")[
            "post_id"
        ]
        ...
        rs_empty_post = db.create_post(rs_a["token"], "rs empty post", "body")[
            "post_id"
        ]
        ...
        rs_clear = reports.report_content(
            rs_b["token"], "post", rs_clear_post, "leans clear"
        )
        rs_sibling = reports.report_content(
            rs_c["token"], "post", rs_clear_post, "sibling flag"
        )
        rs_stay = reports.report_content(
            rs_d["token"], "post", rs_stay_post, "leans suspend"
        )
        rs_tie = reports.report_content(
            rs_d["token"], "post", rs_tie_post, "tie target"
        )
        rs_empty = reports.report_content(
            rs_d["token"], "post", rs_empty_post, "no votes"
        )
        ...
        rs_fresh = reports.report_content(
            rs_e["token"], "post", rs_clear_post, "fresh sibling"
        )
        ...
        assert (
            state[rs_clear["report_id"]]["status"] == "cleared"
            and state[rs_sibling["report_id"]]["status"] == "cleared"
        ...
        assert (
            archived.get(rs_clear["report_id"]) == 2
            and archived.get(rs_sibling["report_id"]) == 2
            and archived.get(rs_fresh["report_id"]) == 2
    ...
    try:
        ...
        assert (
            db.whoami(im_r["token"])["karma"] >= config.MIN_KARMA_MOD
            and db.whoami(im_sus["token"])["karma"] >= 1
        ...
        rep_small = reports.report_content(
            im_r["token"], "post", im_t1["post_id"], "small pool"
        )
        ...
        rep_empty = reports.report_content(
            im_r["token"], "post", im_t2["post_id"], "no votes"
        )
        ...
        rep_susp = reports.report_content(
            im_r["token"], "post", im_t3["post_id"], "leans suspend"
        )
        ...
        os.environ["FORUM_REPORT_SUSPEND_VOTES"] = "2"
        rep_big = reports.report_content(
            im_r["token"], "post", im_t4["post_id"], "big pool"
        )
        ...
        rep_coth = reports.report_content(
            im_r["token"], "post", im_t5["post_id"], "c_other edge"
        )
        for i in range(40):
