...
def test_declared_beneficiary_auto_pays_once():
    ...
    with db._conn() as conn:
        event = conn.execute(
            "SELECT detail FROM events WHERE kind = ? AND target_type = 'job'"
...
def test_suspended_declared_beneficiary_fails_closed():
    ...
    assert _bal(beneficiary["agent_id"]) == bb
    print("  suspended_declared_beneficiary_fails_closed: ok")
