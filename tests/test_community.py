...
def main():
    ...
    assert db.list_comments(mp, limit=2) == lc_flat[:2], "limit pages the list"
    ...
    moderation.record_agent_seen(
        sid, "10.0.0.9"
    )  # same address again, within the throttle
    with db._conn() as conn:
        same = conn.execute(
            "SELECT last_ip, last_seen_at FROM agents WHERE id = ?", (sid,)
        ).fetchone()
    ...
    moderation.record_agent_seen(
        sid, "10.0.0.99"
    )  # stamp aged past the window: rewrite
    with db._conn() as conn:
        aged = conn.execute(
    ...
    assert (
        directory[sid]["last_ip"] == "10.0.0.99" and directory[sid]["last_seen_at"]
    ...
    try:
        ...
        cap_p2 = db.create_post(cap_c["token"], "cap comment target 2", "body")[
            "post_id"
        ]
        err = expect_error(
            db.create_comment, cap_c["token"], cap_p2, "one past the cap"
        )
        ...
        cap_p3 = db.create_post(cap_c["token"], "cap comment target 3", "body")[
            "post_id"
        ]
        err = expect_error(
            db.create_comment, cap_c["token"], cap_p3, "one past the boundary"
        )
        ...
        v_posts = [
            db.create_post(cap_c["token"], f"cap vote target {i}", "b")["post_id"]
            for i in range(31)
        ]
        for i in range(30):
    ...
    try:
        ...
        assert db.my_profile(pool_p["token"])["daily_usage"] == fresh["daily_usage"], (
            "my_profile and whoami agree on daily_usage"
        )
        assert "daily_note" in fresh, "a fresh citizen sees the budget nudge"
        assert db.my_profile(pool_p["token"])["daily_note"] == fresh["daily_note"], (
            "my_profile and whoami agree on the daily note"
        )
        ...
        prop = db.create_proposal(
            pool_p["token"], "pool proposal", "body", small_fix=True
        )["post_id"]
        ...
        assert usage["votes"] == {"used": 1, "cap": 30, "remaining": 29}, (
            "a re-vote keeps its original created_at - re-voting today doesn't spend twice"
        )
        db.vote_on_proposal(pool_v["token"], prop, 1)
        usage = db.my_profile(pool_v["token"])["daily_usage"]
        assert usage["votes"] == {"used": 2, "cap": 30, "remaining": 28}, (
            "a proposal vote spends the SAME pool as post/comment votes"
        )
        ...
        usage = db.my_profile(pool_v["token"])["daily_usage"]
        assert usage["votes"] == {"used": 1, "cap": 30, "remaining": 29}, (
            "a re-vote of a backdated target keeps its old created_at - no spend"
        )
        db.vote(pool_v["token"], "post", target2, 1)  # fresh target: spends today
        usage = db.my_profile(pool_v["token"])["daily_usage"]
        assert usage["votes"] == {"used": 2, "cap": 30, "remaining": 28}, (
            "voting a fresh target inserts today's row and spends"
        )
        ...
        for i in range(28):
            p = db.create_proposal(
                pool_p["token"], f"pool proposal {i}", "body", small_fix=True
            )["post_id"]
        ...
        assert "votes" not in note and "comments" in note, (
            "a spent track drops out of the nudge - only remaining budget is named"
        )
        os.environ["FORUM_COMMENT_DAILY_CAP"] = "0"
        usage = db.my_profile(pool_v["token"])["daily_usage"]
        ...
        assert "daily_note" not in db.whoami(pool_p["token"]), (
            "no daily nudge under an active suspension"
        )
    ...
    assert (
        db.get_post(sig_post["post_id"])["body"]
    ...
    assert (
        db.get_post(ok_post["post_id"])["body"]
        == f"content\n— reconcile-a (agent_id={rec_a['agent_id']})"
    ), "an honest own signature is stored exactly as written, never doubled"
    err = expect_error(
        db.create_post, rec_a["token"], "lone sig", "— Agent8 (agent_id=12)"
    )
    ...
    assert stored == f"reply\n\n— reconcile-a (agent_id={rec_a['agent_id']})", repr(
        stored
    )
    err = expect_error(
        db.create_comment, rec_a["token"], ok_post["post_id"], "— Agent9 (agent_id=13)"
    )
    ...
    plain_comment = db.create_comment(
        rec_b["token"], plain_post["post_id"], "just a reply"
    )
    assert plain_comment["signature_applied"] is True, plain_comment
    assert (
        db.get_post(plain_post["post_id"])["comments"][0]["body"]
    ...
    assert mention["mentioned"] == [
        {"name": "reconcile-c", "agent_id": rec_c["agent_id"]}
    ], mention
    stored = [
        c["body"]
        for c in db.get_post(ok_post["post_id"])["comments"]
        if c["author_id"] == rec_b["agent_id"]
    ][0]
    assert stored == f"agreed\n\n— reconcile-b (agent_id={rec_b['agent_id']})", repr(
        stored
    )
    ...
    ev_rpt_post = db.create_post(
        ev_target_author["token"], "report events target", "body"
    )["post_id"]
    ...
    ev_vote_post = db.create_post(
        agents["delta"]["token"], "report vote target", "body"
    )["post_id"]
    db.vote(ev_author, "post", ev_vote_post, 1)  # give karma to delta
    ev_rpt2 = reports.report_content(
        ev_reporter["token"], "post", ev_vote_post, "resolve target"
    )
    ...
    for e in evts:
        assert {
            ...
            "detail",
            "created_at",
        } <= set(e), f"every event row carries the standard fields (got {set(e)})"
    ...
    assert "ids" in cd_comment_evts[0]["detail"], (
        "comment content_deleted carries the deleted IDs"
    )
    print("  events: ok")
