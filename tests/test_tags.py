...
def main():
    ...
    assert "at least one letter or digit" in expect_error(db.create_tag, t_c, "---"), (
    ...
    try:
        ...
        assert "effective karma" in expect_error(db.create_tag, t_c, "gamma"), (
        ...
        assert "insufficient credits" in expect_error(db.create_tag, t_d, "gamma"), (
    ...
    assert "remove one first" in expect_error(db.apply_tag, t_d, post_d1, "beta"), (
    ...
    p_lock = db.create_proposal(
        tag_l, "Tag-freeze lock", "locked soon", small_fix=True
    )["post_id"]
    ...
    p_merged = db.create_proposal(
        tag_m, "Tag-freeze merged", "merged soon", small_fix=True
    )["post_id"]
    ...
    assert "no tag named 'nope'" in expect_error(db.update_tag, t_d, "nope", "x"), (
    ...
    assert {r["name"]: r for r in db.list_tags()}["delta"][
        "description"
    ] == "the delta tag", "list_tags rows carry the updated description"
    assert (
        db.update_tag(t_d, "delta", "the delta tag")["description"] == "the delta tag"
    ...
    assert "still reserves that name" in expect_error(db.create_tag, t_b, "alpha"), (
    ...
    assert {"haunt", "relic"} <= set(lt), "used tags of a deleted citizen stay listed"
    ...
    assert lt["delta"]["post_author_count"] == 1, "author counts join live posts only"
    with db._conn() as conn:
        anon = conn.execute(
    ...
    assert anon == 2, "only the deleted citizen's applications become anonymous"
    ...
    reborn_post = db.create_post(reborn["token"], "tags: reborn host", "body")[
        "post_id"
    ]
    for voter in (
        w,
