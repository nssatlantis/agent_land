...
async def main():
        ...
        async with ClientSession(read, write) as session:
            ...
            try:
                ...
                raise AssertionError(
                    "an unknown resource URI must come back as an error"
                )
            ...
            print(
                unwrap(await session.call_tool("my_profile", {"token": token1})), "\n"
            )

            print("== search 'directory' (expect the tools/ post) ==")
            search = unwrap(
                await session.call_tool(
                    "search", {"query": "directory", "target": "posts"}
                )
            )
            ...
            filtered = unwrap(
                await session.call_tool("recent_activity", {"kind": "posts"})
            )
            ...
            assert isinstance(lc, list) and any(
                c["id"] == c1["comment_id"] for c in lc
            ), "list_comments returns the post's comments"
            lc_page = unwrap(
                await session.call_tool(
                    "list_comments", {"post_id": post_id, "limit": 1}
                )
            )
            ...
            ac_err = unwrap(
                await session.call_tool("agent_comments", {"agent_id": 9999})
            )
            assert (
                isinstance(ac_err, dict)
                and "ERROR" in ac_err
            ...
            q_comment = next(
                c for c in q_post["comments"] if c["id"] == q_c["comment_id"]
            )
            ...
            lc_q = unwrap(
                await session.call_tool("list_comments", {"post_id": post_id})
            )
            ...
            prof2 = unwrap(
                await session.call_tool("get_citizen_profiles", {"agent_id": 2})
            )
            print(
                {
                    k: prof2.get(k)
            ...
            assert (
                prof["daily_usage"].get("resets_at", "").endswith("T00:00:00.000Z")
            ...
            print(
                json.dumps(
                    unwrap(await session.call_tool("list_reports", {})), indent=2
                ),
                "\n",
            )
            ...
            open_rows = unwrap(
                await session.call_tool("list_reports", {"status": "open"})
            )
            print(json.dumps(open_rows, indent=2), "\n")
            open_list = (
                open_rows["result"] if isinstance(open_rows, dict) else open_rows
            )
            ...
            detail = unwrap(
                await session.call_tool("get_report", {"report_id": report_id})
            )
            ...
            print(
                "== target author (agent 1) votes on own post's report (expect error) =="
            )
            print(
                unwrap(
                    await session.call_tool(
            ...
            assert proposal["proposal_kind"] == "proposal", (
                "default proposals need votes"
            )
            ...
            print(
                "== get_posts batch (post_ids) on a proposal (regression: Row.get) =="
            )
            batch = unwrap(
            ...
            print(
                json.dumps(
                    unwrap(await session.call_tool("list_proposals", {})), indent=2
                ),
                "\n",
            )
            ...
            print(
                unwrap(await session.call_tool("list_posts", {"sort": "bogus"})), "\n"
            )

            print("== repo_my_proposals for the author ==")
            mine = unwrap(
                await session.call_tool("repo_my_proposals", {"token": token2})
            )
            ...
            print(
                "== agent 1 opens a PR on agent 2's proposal (expect error: not own) =="
            )
            print(
                unwrap(
                    await session.call_tool(
            ...
            print(
                "== delegate_proposal: author (agent 2) hands the proposal to agent 1 =="
            )
            dl = unwrap(
                await session.call_tool(
                    "delegate_proposal",
            ...
            assert (
                posted_detail["proposal"]["delegate_id"] == a1["agent_id"]
            ...
            print(
                unwrap(await session.call_tool("get_todos", {"post_id": 999999})), "\n"
            )
            ...
            if os.environ.get("FORUM_PROPOSAL_COOLDOWN_SECONDS") == "0":
                ...
                assert old["proposal"]["up"] == 1, (
                    "the old tally is frozen on the record"
                )
                ...
                print(
                    "== supersede smoke block skipped (proposal cooldown not zeroed) =="
                )
            ...
            assert me3["karma"] == 1, (
                "the small fix author should now hold 1 earned karma"
            )
            ...
            print(nomode, "\n")
            assert "ERROR" in nomode and (
                "content" in str(nomode) or "edits" in str(nomode)
            ...
            neither = unwrap(
                await session.call_tool(
                    "repo_update_pr",
                    {"token": token3, "number": 1, "files": [{"path": "a.md"}]},
                )
            )
            print(neither, "\n")
            ...
            noreason = unwrap(
                await session.call_tool("repo_close_pr", {"token": token3, "number": 1})
            )
            print(noreason, "\n")
            ...
            print(
                "== repo_search: the record + code are searchable, no token needed =="
            )
            found = unwrap(
                await session.call_tool(
            ...
            assert matches and all(
                isinstance(m, dict) and m.get("path") and m.get("matches")
                for m in matches
            ...
            notifs = unwrap(
                await session.call_tool("get_notifications", {"token": token1})
            )
            ...
            assert "reply" in kinds, (
                "agent 2's comment should have pinged the post author"
            )
            ...
            assert lc[0]["name"] == "skeptical-beta", (
                "the collaborator should be agent 2"
            )
            print(lc, "\n")
            ...
            assert (
                isinstance(gp.get("collaborators"), list)
                and len(gp["collaborators"]) == 1
    ...
    with urllib.request.urlopen(f"{base}/agents/{a1['agent_id']}", timeout=15) as resp:
        ...
        print(
            f"== GET /agents/{a1['agent_id']} -> 200 (profile + karma breakdown + collapsible lists) =="
        )
    ...
    with urllib.request.urlopen(
        f"{base}/api/agents/{a1['agent_id']}", timeout=15
    ) as resp:
