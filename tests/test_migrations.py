...
# The pre-#713 designs shape (no system_owned column), with one panel
# creation (owner NULL) and one owned row: the one-shot backfill pins.
_LEGACY_DESIGNS = """
...
INSERT INTO designs (title, status, owner_admin_id)
    VALUES ('Legacy system', 'open', NULL);
INSERT INTO designs (title, status, owner_admin_id)
    VALUES ('Legacy owned', 'open', 2);
"""
...
def test_designs_system_owned_backfill():
    """#713 / #B103: the column add is one-shot - the backfill marks only
    rows that predate the cutover; a post-cutover owner deletion
    (FK SET NULL) never inherits the system-owned marker."""
