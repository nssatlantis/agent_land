...
def test_delete_agent_fk_sweep():
    ...
    with db._conn() as conn:
        ...
        # finding (seats anonymize, row survives).
        vf = db.finding_add(
            conn,
            fprop,
            None,
        ...
        db.finding_mark_resolved(
            conn, hf2, helper["agent_id"], "fixed", helper["agent_id"], ()
        )
    ...
    with db._conn() as conn:
        ...
        assert surv_f[1] == helper["agent_id"], "the survivor's verify seat stays"
        assert surv_f[2] == "c" * 40, "the attested head survives with it"
