...
class _GitFixture:
    ...
    def __init__(self, conflicting: bool):
...
def test_knob_defaults():
    ...
    assert float(config.CI_RUN_SANDBOX_CPUS) == 1.0
    assert config.CI_RUN_SANDBOX_MEMORY_MB == 512
...
def test_gate_bucket_is_branch_kind():
    ...
    ci_runner._ensure_image = lambda tree: "fake:tag"
...
def test_image_tag_tracks_requirements_hash():
    tree = Path(tempfile.mkdtemp(prefix="agentland_ci_tagfx_"))
    (tree / "requirements.txt").write_text("httpx==0.28.1\n")
    tag_a = ci_runner._image_tag(str(tree))
    (tree / "requirements.txt").write_text("httpx==0.28.1\nstarlette==1.6.0\n")
    tag_b = ci_runner._image_tag(str(tree))
...
def _patched_execution(stub_script: str):
    ...
    docker; returns (restore_fn, holder) where holder keeps call counts."""
...
def test_clean_merge_runs_and_reports_shape():
    ...
    fx = _GitFixture(conflicting=False)
    fx.patch_runner()
    restore, holder = _patched_execution(
...
def test_merge_conflict_reports_files_without_running():
    ...
    fx.patch_runner()
    restore, holder = _patched_execution("raise SystemExit('must not run')")
...
def test_native_mode_still_reports_native():
    ...
    restore_exec, _ = _patched_execution("")
    try:
